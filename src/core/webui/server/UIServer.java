...
import core.webui.server.handlers.internals.ToggleRecordHandler;
import core.webui.server.handlers.internals.ToggleReplayHandler;
...
public class UIServer extends IPCServiceWithModifablePort {
	...
	private Map<String, HttpHandlerWithBackend> createHandlers() {
		...
		output.put("/internals/action/add-task", new ActionAddTaskHandler());
		output.put("/internals/action/change-task-group-for-task", new ActionChangeTaskGroupForTaskHandler());
		output.put("/internals/action/compile-task", new ActionCompileTaskHandler());
		output.put("/internals/action/delete-task", new ActionDeleteTaskHandler());
		output.put("/internals/action/edit-source", new ActionEditSourceHandler());
		output.put("/internals/action/move-task-up", new ActionMoveTaskUpHandler());
		output.put("/internals/action/move-task-down", new ActionMoveTaskDownHandler());
		output.put("/internals/action/overwrite-task", new ActionOverwriteTaskHandler());
		output.put("/internals/action/run-compiled-task", new ActionRunCompiledTaskHandler());
		output.put("/internals/action/run-ipc-service", new ActionRunIPCServiceHandler());
		output.put("/internals/action/stop-ipc-service", new ActionStopIPCServiceHandler());
		output.put("/internals/action/switch-task-group", new ActionSwitchTaskGroupHandler());
		...
		output.put("/internals/modify/ipc-service-port", new ModifyIPCServicePortHandler());
		...
		output.put("/internals/toggle/record", new ToggleRecordHandler());
		output.put("/internals/toggle/replay", new ToggleReplayHandler());
